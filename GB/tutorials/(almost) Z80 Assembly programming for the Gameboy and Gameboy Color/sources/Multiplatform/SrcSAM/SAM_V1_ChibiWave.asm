...
ChibiWaveBitSet:
	...
	ld a,&1C
	ld d,%00000011
	call SetSoundRegister	;Reset and Enable sound
	
	ld a,&18
	ld d,%10000010
	call SetSoundRegister	;Envelope: Enable, pattern B - always on
	
	ld a,&02
	ld d,0
	call SetSoundRegister	;Vol - Prep sound register (reg select)
...
Wavedelay:							;Wait for next sample
	...
	ld a,&1C
	ld d,%00000000
	call SetSoundRegister			;Disable Sound
...
Do1BitWavb:
	...
	rlca
Do1BitWavc:
	ld b,a					;Convert data for channel RRRRLLLL
	rlca
	rlca
	rlca
	rlca
	or b					
	ld bc,255				;Port for sound reg data
	out (c),a				;Quickly change volume of selected register
	ret